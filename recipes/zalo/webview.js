  ...
  const getMessages = () => {
    ...
    if (unreadBadge !== null) {
      ...
      switch (true) {
        ...
        case (unreadBadgeFa.match('fa-5') !== null) && (unreadBadgeFa.match('fa-5_Plus') === null) : {
          count = 5;
          break;
        }
        default: {
          // fa-5_Plus
          const convUnread = document.querySelectorAll('.conv-action__unread-v2 > div:not([class*="--noti-disable"]');
          if (convUnread.length === 1)
            count = 6; // 5+
          else
            for (const convUnreadItem of convUnread) {
              switch (convUnreadItem.querySelector('.z-noti-badge__content').classList[1]) {
                case 'fa-1_24_Line': {
                  count += 1;
                  break;
                }
